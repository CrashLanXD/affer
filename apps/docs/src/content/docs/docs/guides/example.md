...
```diff lang="ts" title="art.worker.ts"
+ // Pre-computed cosine and sine arrays for constant 60-degree increments (j * PI / 3)
+const COS_6 = [1.0, 0.5, -0.5, -1.0, -0.5, 0.5];
+const SIN_6 = [0.0, 0.8660254037844386, 0.8660254037844386, 0.0, -0.8660254037844386, -0.8660254037844386];
...
if (isSpiral) {
  const frameRotation = frame * 0.003;
+  const baseAngle = (frame + i * 25) * 0.001 * TWO_PI;
+  const cosBase = Math.cos(baseAngle);
+  const sinBase = Math.sin(baseAngle);

+  const factorX = Math.abs(shapeRadiusX) * (i / loopCount) * cfg.ampX;
+  const factorY = (Math.abs(shapeRadiusY) / Math.abs(cfg.aspectRatio)) * (i / loopCount) * cfg.ampY;
  for (let j = 0; j < 6; j++) {
-    const angle = (j / 6 + (frame + i * 25) * 0.001) * TWO_PI;
-    const localX = Math.cos(angle) * Math.abs(shapeRadiusX) * (i / loopCount) * cfg.ampX;
-    const localY = Math.sin(angle) * (Math.abs(shapeRadiusY) / Math.abs(cfg.aspectRatio)) * (i / loopCount) * cfg.ampY;
+    // Fast inline rotation using trigonometric angle addition identities:
+    // cos(A + B) = cos(A)cos(B) - sin(A)sin(B)
+    // sin(A + B) = sin(A)cos(B) + cos(A)sin(B)
+    const cosRot = cosBase * COS_6[j] - sinBase * SIN_6[j];
+    const sinRot = sinBase * COS_6[j] + cosBase * SIN_6[j];

+    const localX = cosRot * factorX;
+    const localY = sinRot * factorY;

    const worldX = ma * localX + mc * localY + me;
    const worldY = mb * localX + md * localY + mf;
    const dotRadiusX = 8 + 3 * Math.sin(i / 20 + frameRotation);
    const dotRadiusY = dotRadiusX * cfg.aspectRatio;
    this.pushInstance(worldX, worldY, targetIdx, dotRadiusX, dotRadiusY, finalRot, shapeTypeInt);
  }
}
```